...
# ntile() outputs a rough rank, which breaks the input vector into n buckets
kickstarter_not_live <- kickstarter_not_live %>%
  mutate(usd_goal_real_tier = ntile(kickstarter_not_live$usd_goal_real, num_tiers))
```
...
Our test statistic was 20009, which has a chi-square distribution with 9 degree of freedom under $H_0$. This corresponds to a p-value less than 2.2e-16. So our decision is to reject $H_0$, and there is sufficient evidence that there is a relationship between a project's goal and a project's state.

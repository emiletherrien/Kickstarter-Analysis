...
results while excluding the 1's and 0's. We plotted bar charts to show the 
proportion of successful to unsuccessful ("other") projects, faceted by category. 
By using a ratio of successes to others, we were able to visualize the relative 
...
Interestingly, the visualization shows that more categories failed in raising 
enough money to meet their goals than those that achieved their monetary goal. 
Only the categories of Comics, Dance, and Theatre had more successess than 
failures according to our visualizations.
...
The aim of this project was to explore which variables influence the success of 
a Kickstarter project. We did so by analyzing which types of projects are more 
likely to be funded. We wanted to know whether the project goal (in USD) was 
associated with its chance of success; what amount of money asked for would be 
more likely to succeed; whether the project category was associated with its 
chance of success; and which category of project is more likely to succeed.

For our exploratory data analysis (EDA), we first created a table that showed the 
count of each Kickstarter Project State. This table showed us that the most 
common outcome for a project in this data set is 'failure.'

We then modeled the distribution of project states in a bar plot, again finding 
that more projects failed than succeeded.

Finally, we calculated the mean goal amount and the mean pledged amount, and 
plotted the two on a bar plot. This EDA shpowed us that the average funding 
pledged was $9,058.92, with a standard deviation of $90,973.34. In comparison, 
the average funding goal was $45,454 with a standard deviation of $1,152,950. 
The observed mean average funding goal was approximately five times higher than 
the average.

Our EDA showed us that the odds are stacked against most kickstarter projects in 
terms of being likely to successfully reaching their funding goal. Given that our 
EDA used all of the data in the kickstarter data set (except forthe _live_ 
observations), our observations were relatively reliable and valid.

However, it would have also been helpful to plot a bar chart of the count of the 
main project categories to see which ones were most popular. This would have been
good to know going into the deeper analysis section, as it would have given us an 
idea of which main categories were most popular in terms of pitching kickstarter
projects. 


Farzeen: This will require a summary of what you have learned about your research
question along with statistical arguments supporting your conclusions. 
 
 In our analysis, we have basically answered few questions such as the relationship
 between the funding tiers ( from 1 till 7) and the success-rates of these projects.
 We defined success_state as successful if these projects reached the funding
 goal.We also studied various other factors that influence the success_state of
 the project such as categories of the projects.
 
 For our first analysis, we divided the funding amount into 7 tiers so that it
 is easier for us to compare the funding money. We used the chi-squared test to
 check if the two variables we are testing - usd_goal_real_tier 
 (just referred to as Tier in future) and success_rate-
 independent.From our results we concluded that there is enough evidence to suggest 
 that a project's Tier is related to its success rate.The next question is how is
 project's Tier related to the success rate of the project.For this part, we used 
 the projects with usd_goal_real_tier =1 as a reference level to our linear 
 regression logistic model to see how other Tier's compare to this model.We calculated
 the odds of success and found out that Tier 2 comes closest in the competition for
 high success_state but has 0.76 times the odds of success for the Tier 1 projects.
 We also calculated the probability of success for each tier and found out that Tier 1
 indeed has the highest probability of success.
 
 This makes complete sense because it is more likely a person is able to complete
 the funding for the project if the funding goal is low.
 
 The second hypothesis was how are the categories of projects influencing the
 success_state of the project.We used a Chi-squared test so we could analyze if the
 variables tested - main category and success_rate - are independent of one another 
 in this data set. In other words, running a Chi-squared test helps us evaluate 
 our hypothesis; that there is an association between project category and project 
 success rate. Now we can see which categories are most likely to be funded. We use
 a logistic model and keep technology category as our reference level. We were
 surprised to find that dance is the category most likely to be funded.
